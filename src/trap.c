...
void TrapKernel(UserContext *user_context)
{
    ...
    case YALNIX_CVAR_SIGNAL:
        ...
        r_value = Y_CvarSignal((int)user_context->regs[0]);
        break;
    case YALNIX_CVAR_BROADCAST:
        TracePrintf(1, "TRAP CVAR_BROADCAST REG; 0: %d\n", (int)user_context->regs[0]);
        r_value = Y_CvarBroadcast((int)user_context->regs[0]);
        break;
    case YALNIX_CVAR_WAIT:
        TracePrintf(1, "TRAP CVAR_WAIT REG; 0: %d, 1: %d\n", (int)user_context->regs[0], (int)user_context->regs[1]);
        r_value = Y_CvarWait((int)user_context->regs[0], (int)user_context->regs[1]);
        break;
    case YALNIX_RECLAIM:
        TracePrintf(1, "TRAP RECLAIM REG; 0: %d\n", (int)user_context->regs[0]);
        r_value = Y_Reclaim((int)user_context->regs[0]);
        break;
